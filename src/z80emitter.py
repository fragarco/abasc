...
class z80Emitter:
    ...
    def _emit_code(self, line: str="", indent=0):
        pad = ""
        for _ in range(indent): pad = pad + " "
        self.code = self.code + f"{pad}{line}\n"

    def _emit_data(self, line: str="", indent=0):
        pad = ""
        for _ in range(indent): pad = pad + " "
        self.data = self.data + f"{pad}{line}\n"

    def _emit_head(self):
        self._emit_code("; FILE GENERATED BY BASC COMPILER")
        self._emit_code("; DESIGNED TO BE ASSEMBLED BY ABASM")
        self._emit_code(";")
        self._emit_code(f"org   {hex(self.org)}")
        self._emit_code()
        self._emit_code("_init_:")
        self._emit_code(f"call   {FWCALL.TXT_INITIALISE}    ; TXT_INITIALISE", 4)
        self._emit_code(f"call   {FWCALL.TXT_CLEAR_WINDOW}    ; TXT_CLEAR_WINDOW", 4)
        self._emit_code()
        self._emit_code("_code_:")
        self._emit_data("_data_:")
        ...
        for sym in self.symtable.syms:
            ...
            if entry.symtype == SymType.Variable:
                if entry.exptype == AST.ExpType.Integer:
                    self._emit_data(f"{entry.label}:")
                    self._emit_data(f"dw    0")
                if entry.exptype == AST.ExpType.String:
                    self._emit_data(f"{entry.label}:")
                    self._emit_data(f"ds    255")
                if entry.exptype == AST.ExpType.Real:
                    self._emit_data(f"{entry.label}:")
                    self._emit_data(f"db    0,0,0,0,0")
