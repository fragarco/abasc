...
class CPCEmitter:
    ...
    def _emit_head(self):
        ...
        self._emit_code(";", 0)
        self._emit_code("$LIMIT$", 0)
        self._emit_code(f"org     {FWCALL.LOW_LIMIT}", 0)
        ...
        self._emit_code(f"org     &{hex(self.org)[2:]}", 0)
        self._emit_code()
    ...
    def _emit_code_end(self):
        ...
        if self.memlimit < 99999:
            self.code = self.code.replace("$LIMIT$", f"limit   &{hex(self.memlimit)[2:]}")
        else:
            self.code = self.code.replace("$LIMIT$", "")
    ...
    def _emit_CLEAR(self, node:AST.Command):
        ...
        self._emit_code(f"ld      de,{FWCALL.MATH_SET_ANGLE_MODE}", info="SET_ANGLE_MODE")
