...
class CPCEmitter:
    ...
    def _reserve_memory_de(self, nbytes: int):
        ...
        self._emit_code(f"ld      bc,{nbytes}", info="bytes to reserve")
        self._emit_code("call    rt_malloc_de", info="DE points to empty mem")
        self.free_tmp_memory = True
        self.reserved_tmp_memory += nbytes
    ...
    def _movelong_accum1(self):
        """ Moves long number in (HL) to rt_math_accum1"""
        self._emit_code("ld      de,rt_math_accum1")
        self._emit_code("ld      bc,5")
        self._emit_code("ldir")
    ...
    def _movelong_temp(self):
        """ 
        Copies the current long number pointed by HL to a new
        temporal memory location. Returns in Hl that address. 
        """
        self._reserve_memory_de(4)
        self._emit_code("push    de")
        self._emit_code("ldir")
        self._emit_code("pop     hl")
    ...
    def _emit_preamble(self):
        ...
        self._emit_head()
        self._emit_head("; DYNAMIC MEMORY AREA, USED TO STORE TEMPORAL VALUES",0)
        self._emit_head("_memory_next: dw _memory_start", 0)
        self._emit_head("_memory_start:", 0)
        self._emit_head()
    ...
    def _emit_vardecl(self, entry: SymEntry):
        ...
        elif entry.exptype == AST.ExpType.Long:
            self._emit_data(f"{entry.label}: defs 4")
    ...
    def _emit_arraydecl(self, entry: SymEntry):
        ...
        elif entry.exptype == AST.ExpType.Integer:
            self._emit_data(f"{entry.label}: db   {4*items}")
        elif entry.exptype == AST.ExpType.String:
    ...
    def _long(self, n: int) -> bytearray:
        return bytearray((n).to_bytes(4, 'little'))
    ...
    def _emit_CINT(self, node:AST.Function):
        ...
        if node.args[0].etype != AST.ExpType.Integer:
            if node.args[0].etype == AST.ExpType.Real:
                ...
                self._emit_import("rt_math_call")
                self._movelong_accum1()
                self._emit_code(f"ld      ix,{FWCALL.MATH_BIN_TO_REAL}", info="MATH_BIN_TO_REAL")
                self._emit_code("call    rt_math_call")
                self._emit_code(f"ld      ix,{FWCALL.MATH_REAL_TO_INT}", info="MATH_REAL_TO_INT")
                self._emit_code("call    rt_math_call")
    ...
    def _emit_CLEAR(self, node:AST.Command):
        ...
        of calculation. 
        """
        # Only the action to set RAD is performed here
        self._emit_import("rt_math_call")
    ...
    def _emit_CLONG(self, node:AST.Function):
        ...
        if node.args[0].etype == AST.ExpType.Real:
            self._emit_import("rt_math_call")
            self._moveflo_accum1()
            self._emit_code(f"ld      ix,{FWCALL.MATH_REAL_TO_BIN}", info="MATH_REAL_TO_BIN")
            self._emit_code("call    rt_math_call")
            self._movelong_temp()
        ...
        for var in node.args:
            if isinstance(var, AST.Array):
                ...
                if entry is not None:
                    ...
                    elif var.etype == AST.ExpType.Long: mem = mem * 4
    ...
    def _emit_HIMEM(self, node:AST.Function):
        ...
        """
        # In our case we use this to return address where the compiled program
    ...
    def _emit_assigment(self, node: AST.Assignment):
        ...
        if var is not None:
            ...
            elif var.exptype == AST.ExpType.Long:
                if isinstance(node.target, AST.ArrayItem):
                    self._emit_code("push    hl")
                    self._emit_arrayitem_ptr(node.target)
                    self._emit_code("pop     de")
