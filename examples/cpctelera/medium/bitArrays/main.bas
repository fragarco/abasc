...
label MAIN
  ...
  '
  array1ptr = @label(array1)
  array2ptr = @label(array2)
  array4ptr = @label(array4)
  while 1
    ' First, erase all contents of our 3 arrays,
    ' setting all their bits to 0
    call cpctMemset(array1ptr, 0, 10)
    call cpctMemset(array2ptr, 0, 20)
    call cpctMemset(array4ptr, 0, 40)
    ...
    for i=0 TO 79
      ' Set Bit i to 1
      call cpctSetBit(array1ptr, 1, i)

      ' Print the complete array at the top of the screen
      call printArray(CPCT.VMEMSTART, array1ptr, 80, 0) 
      
      ' Reset again the bit to 0 an iterate
      call cpctSetBit(array1ptr, 0, i)
    next
    ...
    for j=3 to 1 step -1 
      for i=0 to 79
        ...
        call cpctSet2Bits(array2ptr, j, i)

        ' Print the complete array
        call printArray(&C0A0, array2ptr, 80, 1)

        ' Reset the value of the item to 0 again
        call cpctSet2Bits(array2ptr, 0, i)
      next
    next
    ...
    for j=0 to 15 
      for i=0 to 79
        ...
        call cpctSet4Bits(array4ptr, value, i)
        call printArray(&C140, array4ptr, 80, 2)
      next
    next
...
label array1: ASM "defs 10" ' Array of 80 1-bit elements (10 bytes)
label array2: ASM "defs 20" ' Array of 80 2-bit elements (20 bytes)
label array4: ASM "defs 40" ' Array of 80 4-bit elements (40 bytes)