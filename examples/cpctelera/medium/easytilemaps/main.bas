...
sub application
   ...
   call cpct_disableFirmware()     ' Firmware must be disabled for this application to work
   ...
   call cpctetmSetTileset2x4(g_tileset)

   ' Indefinitely draw the tilemap, listen to user input, 
   ' do changes and draw it again
   while 1
      call drawScreenTilemap(&scr)   ' Redraws the tilemap
      call readKeyboardInput(&scr)   ' Waits for a user input and makes associated changes
   wend
end sub
...
label MAIN
   ...
   call cpctSetStackLocation((void*)0x8000)
